...
- **Fixed:** event canonicalization preserves only validated ref/source; public event/plan/signup handoff carries validated ref/source/kind/target; browser signup persists a 24-hour first touch through callbacks and calls the authenticated shared referral RPC only after compliance.
...
- **Verified/boundary:** the hotfix-based rebase passes Node 9/9, desktop/mobile Playwright 180/180, build validation, deploy-omission regression, diff check, and branch-range gitleaks; exact-head independent rereview remains. PR #93 is open; no migration, deploy, secret, production write, or public release occurred.
