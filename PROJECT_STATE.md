...
- Website/app separation is LIVE (2026-07-08, Codex): trydropapp.com is the standalone public website; the Expo web proxy is retired. All global-nav `/app/login` links were removed; `/login`, `/login.html`, `/app`, `/app/...`, and `app.trydropapp.com/*` redirect to `download.html`. Deployed Pages `1cab876c.drop-site.pages.dev`; deployed Worker `drop-app-path` version `0b7f6161-e741-482c-aa07-deafaa7d257e`.
...
### Exact next step
1. Founder QA: hard-refresh `https://trydropapp.com`, confirm the header has no `Log in` link, and spot-check `/download.html`, `/events.html`, one artist detail, and one venue detail at 390/768/1440.
2. Founder QA: open `https://trydropapp.com/app/`, `https://trydropapp.com/app/login`, and `https://app.trydropapp.com/`; each should land on `https://trydropapp.com/download.html`, not an Expo web app.
...
- `trydropapp.com/app*` and `app.trydropapp.com/*` → Worker **drop-app-path** redirects to `https://trydropapp.com/download.html`; no Expo web proxy is active on public routes.
- DNS (zone 5ac5024f…): apex+www CNAME → drop-site.pages.dev; app host remains proxied so the Worker route can catch legacy links.
- Verified live: all 12 pages 200, /link 200, /legal/* 301s, /event/<uuid> serves event page (200 rewrite + path-parsed id), AASA application/json at root, www→apex 301. Browser check: h1 renders, 24 live event cards, body scrolls (no app overflow:hidden), zero page errors. 2026-07-08 check: `/app/`, `/app/login`, and app subdomain routes 302 to `/download.html`; broad `/app*` non-segment paths pass through to the website origin.
