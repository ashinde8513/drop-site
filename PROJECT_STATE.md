...
### Exact next step
1. **Founder: merge the parity drop** — 3 PRs together: this repo's nav+card parity PR (drop-site #10) + login-loop worker fix PR (drop-site #9, worker already deployed) + Drop-App web-parity PR (#138, CI green required). Then in this repo: `git checkout main && git pull && npm test && rsync -a _headers _redirects *.html data.js site.js site.css favicon.svg og-image.png og-image-link.png robots.txt sitemap.xml llms.txt dist/ && npx wrangler pages deploy dist --project-name=drop-site --branch=main`. Drop-App main push auto-deploys /app.
2. Founder QA after deploy: hard-refresh trydropapp.com (header = Events·Venues·Artists, pill styles) + login → /app Discover shows H1 + hero search + Happening tabs + genre tiles + venues + footer; eyeball card parity at 390/768/1440.
