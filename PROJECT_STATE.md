...
## SESSION LOCK
**Status:** REVIEW — Phase 2 PR #93 rebased onto the Android DAL hotfix
...
- Working on: preserve the exact reviewed Android association and deploy validator while reconciling held Phase 2 website PR #93 on `agent/phase2-share-attribution-20260817`.
...
- **PHASE 2 SHARE ATTRIBUTION PR #93 OPEN (2026-08-17; DELIVERY GATED):** canonical event redirects retain only validated `ref`/`src`; public signup links carry UUID referrer/target, route-derived event/plan/signup kind, and the fixed source catalog into the browser app. Browser signup stores first touch for at most 24 hours; same-browser password/email/OAuth completion can retain it through local storage as best-effort attribution without duplicating it into Auth metadata. Fresh-browser email confirmation fails closed without referral credit: the hosted template does not carry attribution, and callback-time query reconstruction cannot credit an already-created account. Eligible same-browser state records only after compliance through the shared authenticated RPC. `/plan/<uuid>` rewrites in place to a generic private-plan fallback that retains the UUID without rendering roster, meetup, event, venue, date, or friend query data. The branch is based on the approved DAL hotfix and therefore carries the same exact package/fingerprint and deploy-boundary validator; it cannot restore the former empty association. Rebased Node 9/9, Playwright 182/182, build validator, deploy-omission regression, diff check, and branch-range gitleaks pass; exact-head rereview remains. No migration apply, website deploy, secret, production mutation, or public release occurred.
...
### Exact next step
- **PR #95 is independently reviewed and CI-green but its merge/automatic production deploy needs explicit founder approval. Keep PR #93 held on that exact hotfix base, reconcile its Phase 2 browser overlap, and deliver compatible clients before requesting exact approval for shared migration `20260817085038_persist_referral_source_attribution.sql` SHA-256 `e1ce0ac530e3e86bde957613561f2dd2b94d7b5c7ab4e861cb4b51503a492c20`. Keep the migration, Android build, Play release, and device acceptance separately gated.**
...
- Historical state at that cutover, now superseded: `.well-known/assetlinks.json` then kept `app.drop.mobile` + a TODO certificate fingerprint because Android was deferred. The current source uses `app.resonanceventures.drop` and the exact Play App Signing fingerprint. `APP_STORE_URL` constants (site.js/app.js/link.html) were empty then; the current site uses the live App Store listing.
...
- **Verified/boundary:** the hotfix-based rebase passes Node 9/9, desktop/mobile Playwright 182/182 including fresh-browser no-credit behavior, build validation, deploy-omission regression, diff check, and branch-range gitleaks; exact-head independent rereview remains. PR #93 is open; no migration, deploy, secret, production write, or public release occurred.
