...
Approved production test accounts may prove narrowly scoped client behavior only when every temporary row is named and exact cleanup is read back. The completed block/unblock profile-privacy and crew create/delete journey does not promote unrun plans/media/account-utility ownership cases to Passed. Mobile merge, owner-only Sites delivery, native build/submission, OTA, external TestFlight access, and App Store production release remain independent approval boundaries. The consumed database approval authorizes none of these actions.

The merge and owner-only Sites gates were separately consumed on 2026-07-29: mobile PR #290 merged as `21ce9b6`, producing `release-train-31-21ce9b6`, and Sites v12 deployed exact source `7162be2`. On 2026-07-30, a separate approval consumed the native `build-and-submit` gate for that exact tag, producing TestFlight Build 16. No OTA, external Beta Review/group change, production website merge, or App Store production release was authorized. The earlier `a904ccc…` fingerprint was not generated with the exact tag's locked dependency tree and is superseded; the workflow's exact command after `npm ci` returned `d42ed9dc…`, matching Build 16. Future releases retain the same independent approval boundaries and must compute fingerprints only after the locked install.
