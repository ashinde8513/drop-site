...
The website keeps referral recording disabled by default until a separately reviewed runtime capability explicitly confirms that the production RPC contract is live. While disabled, valid first-touch state stays browser-local until it expires; malformed, expired, self, and older-account state still clears. Enabling the capability requires source review after live contract verification, and RPC failure preserves valid state for a later bounded retry.
