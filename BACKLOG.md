...
- [ ] **Security approval / hosted evidence:** exact migration `20260729182349_social_mutation_contracts.sql` and requester-denial/recipient-success tests are locally verified; approve an authorized non-production apply and seeded hosted run before any production request (`WEB-DEF-FRIEND-AUTH-001`).
- [ ] **Security approval / hosted evidence:** the same exact migration derives server identity/time, enforces authored/8h/24h event windows, and fixes reverse-direction block privacy in local contracts; prove it against seeded non-production accounts (`WEB-DEF-CHECKIN-AUTH-001`).
- [ ] **Live evidence:** provision an authorized non-production Supabase branch/project and seeded owner/non-owner/blocked accounts; the connected owner-preview smoke already passed, but hosted multi-account RLS/RPC and mobile viewport evidence remain (`WEB-DEF-TEST-001`, `WEB-BLK-BROWSER-001`).
- [ ] **Backend approval / hosted evidence:** both clients now use the locally verified atomic `replace_crew_members` RPC; approve the exact migration in non-production and run owner/non-owner/failure-injection checks before coordinated release (`WEB-DEF-CREW-001`).
