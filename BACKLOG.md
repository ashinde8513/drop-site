...
- [ ] **Production security approval:** founder explicitly approves exact file `20260729182349_social_mutation_contracts.sql` for production project `ebccwnkmsnhbljxxxdej`; then Backend/Security applies and reads back policies, trigger, grants, and safe smoke before either client releases (`WEB-DEF-FRIEND-AUTH-001`, `WEB-DEF-CHECKIN-AUTH-001`, `WEB-DEF-CREW-001`).
- [ ] **Full browser authorization evidence:** targeted hosted social contracts and owner-preview console smoke pass, but Free-plan QA is not a full schema clone. After backend apply, run approved-account owner/non-owner/blocked browser journeys; otherwise provision a Pro/full-schema non-production target (`WEB-DEF-TEST-001`, `WEB-BLK-BROWSER-001`).
