...
- [x] Build the canonical website QA inventory and Excel mirror, preserve founder/Apple requirement wording and IDs, merge current `origin/main`, fix safe client mutation/accessibility/race defects, and verify exact commit `a081e702` with 291 passed/1 intentional skip (2026-07-29). Isolated branch only; owner-only preview and production remain unchanged.
- [ ] **Security approval:** replace general friendship status updates with a recipient-only, pending-only server contract and add requester-denial/recipient-success DB tests (`WEB-DEF-FRIEND-AUTH-001`).
- [ ] **Security approval:** replace client-clock Live Mode writes with a server-time event-window check-in contract and forged-time/friend/block DB tests (`WEB-DEF-CHECKIN-AUTH-001`).
- [ ] **Live evidence:** provision seeded non-production owner/non-owner/blocked accounts and a connected owner browser; verify RLS, OAuth completion, owner-only access, desktop/mobile screenshots, and hosted console output (`WEB-DEF-TEST-001`, `WEB-BLK-BROWSER-001`).
- [ ] **Backend approval:** add one atomic crew-membership replacement RPC plus failure-injection tests before changing the client (`WEB-DEF-CREW-001`).
...
- [ ] Measure signed-in cold load and route interaction on target desktop/mobile before deciding whether the 765.14 kB main chunk needs route-level lazy loading (`WEB-DEF-PERF-001`).
