...
- [ ] **Coordinated client release:** production atomic crew RPC is live and immutable mobile/website candidates plus deterministic/approved-account evidence are prepared. Deployed clients remain on the legacy multi-write path; require separate exact mobile merge, later exact-tag OTA, and saved owner-only Sites version authorizations (`WEB-DEF-CREW-001`, `WEB-DEF-DEPLOY-001`).
