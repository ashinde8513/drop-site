...
- [ ] Deliver the reviewed Android Digital Asset Links hotfix through the normal `main` auto-deploy, then verify the apex response byte-for-byte. Source now has the exact Play App Signing fingerprint for `app.resonanceventures.drop`; production is still stale until delivery, `www` remains intentionally excluded while it redirects, and physical acceptance waits on a Play-signed internal release.
- [ ] Deliver independently reviewed Phase 2 share attribution from `agent/phase2-share-attribution-20260817`: reconcile the clean-applying `app/app.js` overlap with Phase 1 Facebook Login, then coordinate compatible clients before separately requesting exact approval for shared migration `20260817085038_persist_referral_source_attribution.sql` SHA-256 `e1ce0ac530e3e86bde957613561f2dd2b94d7b5c7ab4e861cb4b51503a492c20`.
- [ ] After separate approvals create the EAS upload credential, production AAB, and Play Internal release, then verify package `app.resonanceventures.drop` and only `/`, `/event/*`, `/plan/*`, `/reset-password` on that exact Play-signed installed artifact.
