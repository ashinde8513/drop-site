...
- [ ] **Coordinated client release:** production atomic crew RPC is live, but deployed clients remain on the legacy multi-write path. Prepare immutable mobile/website heads, pass required deterministic/self-hosted gates, and obtain separate merge/OTA/owner-only Sites authorization before delivery (`WEB-DEF-CREW-001`, `WEB-DEF-DEPLOY-001`).
- [ ] **Full browser authorization evidence:** targeted social contracts are live and owner-preview console smoke passes, but no approved full-schema multi-account browser target was exercised. Before client release, run explicitly approved owner/non-owner/blocked journeys; otherwise provision a Pro/full-schema non-production target (`WEB-DEF-TEST-001`, `WEB-BLK-BROWSER-001`).
