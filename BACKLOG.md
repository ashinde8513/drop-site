...
- [ ] **Exact-tag native mobile build:** tag `release-train-31-21ce9b6` passes release-ref provenance, but runtime `a904ccc…` differs from latest production build runtime `d42ed9d…`; obtain Founder authorization for `production-release` action `build-only` or `build-and-submit`, including EAS usage and any TestFlight submission. Do not dispatch OTA (`WEB-DEF-CREW-001`).
