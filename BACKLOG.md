...
- [x] Build the canonical website QA inventory and Excel mirror, preserve founder/Apple requirement wording and IDs, merge current `origin/main`, fix safe client mutation/accessibility/race defects, and verify corrected behavior commit `34630b5` with 295 passed/1 intentional skip plus 20/20 mobile-WebKit repetitions of the deletion-focus and event-action regressions (2026-07-29). Isolated branch only; owner-only preview and production remain unchanged.
- [x] **Hosted social authorization evidence:** exact migration hash `3c3857ed…431e89f` applied only to $0/month QA project `jrlqozbbrbivmzazuaic`; the 14-case anonymous/requester/recipient/owner/non-owner/friend/nonfriend/both-block-direction/time-window/rollback matrix passed twice with zero residue (2026-07-29).
- [x] **Production social security contracts:** founder-approved exact file `20260729182349_social_mutation_contracts.sql` applied once to production project `ebccwnkmsnhbljxxxdej` as version `20260729200633`; migration/policy/trigger/RLS/grant/function/project/advisor readback passed with zero advisor ERRORs (2026-07-29; `WEB-DEF-FRIEND-AUTH-001`, `WEB-DEF-CHECKIN-AUTH-001` fixed).
- [x] **Authorized client delivery:** merged Drop-App PR #290 exact head `16f9463` with merge commit `21ce9b6`, verified tag `release-train-31-21ce9b6` and all controller/closeout/tagged-web-deploy runs, and deployed owner-only Sites v12 exact source `7162be2` with access limited to `ashinde8513@gmail.com` (2026-07-29). No OTA or production website merge/deploy occurred.
- [x] **Exact-tag native mobile build and TestFlight submission:** Founder-authorized `production-release` run `30520238695` built exact tag `release-train-31-21ce9b6` as iOS 1.0.1 (16), EAS Build `0cc12512-5999-4c40-b595-671ac2991a9a`; exact submission `914d5987-682d-42ff-bb35-388a199926cb` finished and Apple confirmed availability to test (2026-07-30). The authoritative post-`npm ci` fingerprint is `d42ed9dc…`; the earlier `a904ccc…` preflight was superseded. No OTA or App Store production release occurred.
- [ ] **Build 16 physical-device QA:** install TestFlight Build 16 and run Settings provenance, atomic crew replacement/rollback, VoiceOver/Dynamic Type, Universal Links, provider callbacks, photo-library, offline/retry, interruption, and relaunch-persistence checks before any direct-grant cutover (`BLK-QA-002`, `WEB-DEF-CREW-001`).
- [ ] **Production website release remains separate:** keep `drop-site` PR #28 unmerged unless Founder authorizes its then-exact head and production route/access decision (`WEB-DEF-DEPLOY-001`).
- [ ] **Full browser authorization evidence:** approved social/profile/crew client journeys pass with exact cleanup, but plans/media/account-utility owner/non-owner cases remain unproven. Run them on an exact delivered candidate with explicit production-test-account approval, or provision a Pro/full-schema non-production target (`WEB-DEF-TEST-001`, `WEB-BLK-BROWSER-001`).
- [ ] **CI approval and stabilization:** invoke `npm run typecheck:webapp` and cap Playwright at two workers when `CI` is present, then require two consecutive zero-flake full runs without changing runner labels or deploy conditions (`WEB-DEF-CI-001`, `WEB-DEF-CI-FLAKE-001`). Evidence head `a91098a` passed locally 295/1; run `30517391362` failed once under five-worker WebKit load and passed its single retry with 3 recovered flakes.
- [ ] Measure signed-in cold load and route interaction on target desktop/mobile before deciding whether the 764.85 kB main chunk needs route-level lazy loading (`WEB-DEF-PERF-001`).
