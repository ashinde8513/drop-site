...
- [ ] Deliver the reviewed Android Digital Asset Links hotfix through the normal `main` auto-deploy, then verify the apex response byte-for-byte. Source now has the exact Play App Signing fingerprint for `app.resonanceventures.drop`; production is still stale until delivery, `www` remains intentionally excluded while it redirects, and physical App Links acceptance remains untested because no Android test device is available. Play-signed 1.0.2 (2) is already active in Internal Testing.
