...
- **Register:** product — the **browse-first discovery website** (AXS-style; see `AXS_MODEL.md`). One website, two views — **never call it a marketing page / landing page** (founder, 2026-07-18): the **signed-out view** (open browse at trydropapp.com) and the **signed-in view** (the Prism SPA at `app.trydropapp.com` / `/app` — `app/index.html` + `app/app.js`, ported from the "Website design prompt" claude.ai design, not Expo web). Content is the hero. Design-first: iterate in the design project, founder QAs there, then port.
